#!/usr/bin/env python3
#
# Engel Algılayınca durup drive sistemine bildiri yolluyor
# Yazan Mehmet Ali Şahin
    ...
    *******************************************************************************"""
    ...
    def detect_obstacle(self):
        ...
        if obstacle_distance > safety_distance:
            ...
            text = "Obstacles detected, robot stopped."

            # Create a text file and write the content to it
            with open('obstacle_detection.txt', 'w') as f:
                f.write(text)

            # Upload the file to Google Drive
            file = drive.CreateFile({'title': 'obstacle_detection.txt'})
            file.Upload()
            self.get_logger().info('Obstacle detection log uploaded to Google Drive.')
